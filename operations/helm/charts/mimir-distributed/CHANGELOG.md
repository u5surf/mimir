...
* [ENHANCEMENT] Add ability to manage PrometheusRule for metamonitoring with Prometheus operator from the Helm chart. If `serviceMonitor` is enabled, rules standard to Mimir will be deployed. The accompanying alerts are disabled by default but can be created with `serviceMonitor.mimirAlerts` set to `true`. To disable the default rules, set `mimirRules` to `false`. #2134 #2609
